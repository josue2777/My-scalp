...
bool IsAuthorized()
  {
   datetime start = D'2026.03.07 00:00';
   datetime end = D'2026.03.21 23:59:59';
...
int OnInit()
  {
   if(!IsAuthorized())
     {
      Alert("❌ Bot non autorisé - Hors période d'utilisation (07/03/2026 - 21/03/2026)");
      return INIT_FAILED;
     }
...
void OnTick()
  {
   if(!IsAuthorized())
     {
      Print("Période d'utilisation expirée. Désactivation du bot.");
      CloseAllOrders();
      ExpertRemove();
      return;
     }
